...
const { Timestamp } = require("firebase-admin/firestore");  // Neuer Import
admin.initializeApp();
...
exports.createDailyChallengeExperte = onSchedule("0 0 * * *", async (event) => {
  ...
  // Bestimme das heutige Datum im Format YYYY-MM-DD
  const now = new Date();
  ...
  if (!docSnap.exists) {
    ...
    await challengeRef.set({
      ...
      answer: sum.toString(),
      created_at: Timestamp.now(),  // Verwende den importierten Timestamp
    });
